# Implementation Plan: Reconciliation Workbench Comparison Tables

**Branch**: `020-reconciliation-workbench-comparison-tables` | **Date**: 2026-05-24 | **Spec**: [spec.md](./spec.md)

**Input**: Feature specification from `/specs/021-reconciliation-comparison-tables/spec.md`

## Summary

Improve the existing browser reconciliation workbench page by replacing the
single combined reconciliation table presentation with visible analyst-readable
comparison tables for shared facts and shared values. The increment is
presentation-only: it uses existing deterministic slices, approved committed
sample artifacts, existing cross-slice reconciliation evidence, DD/fallback
metadata, and mocked display context while preserving browser-only behavior,
sql.js boundaries, output adapter behavior, and existing slice contracts.

## Technical Context

**Language/Version**: TypeScript in the existing browser-only web app and shared deterministic packages.

**Primary Dependencies**: Existing Vite web workspace, existing `apps/web/src/app/reconciliationWorkbenchSlice.ts`, existing `apps/web/src/pages/ReconciliationWorkbenchPage.ts`, existing `@pbgc/shared` shared-fact and shared-value reconciliation helpers, existing BSRS/V1/VE/valuation output packages, approved sample fixtures already committed in `packages/tests`, and current DD/fallback mapping evidence.
...
**Testing**: Focused workbench tests for shared-facts table state and markup, shared-values table state and markup, normalized-value display where applicable, status/severity display, deterministic repeated ordering, no-real-person-data guardrails, and existing output/reconciliation preservation checks.
...
**Performance Goals**: The fixed approved-sample workbench remains responsive in normal local browser review and renders comparison tables without adding multi-case search, import, export, or recalculation workflows.
...
**Scale/Scope**: One visible workbench page for the existing fixed approved sample. Displays a shared-facts table and a shared-values table using current reconciliation evidence. Does not add case selection, editing, import, export, recalculation, multi-case workflow, or new adapters.
...
- Browser-only static runtime: PASS. The comparison-table increment stays inside the existing static browser app and introduces no server calls, hosted APIs, remote calculation services, telemetry, or network-loaded business logic.
...
- Traceability: PASS. Each displayed comparison row must expose compared slices, fields, raw values, normalized values where applicable, status, severity where applicable, mapping/fallback basis where available, and stable evidence basis.
...
```text
specs/021-reconciliation-comparison-tables/
├── plan.md
├── research.md
├── data-model.md
├── quickstart.md
├── contracts/
│   └── reconciliation-comparison-tables.md
├── checklists/
│   └── requirements.md
└── spec.md
```
...
page renderer, tests, and styles to expose separate shared-facts and
shared-values comparison tables. Keep engine modules, output adapters, database
migrations, seeds, schemas, and deterministic slice contracts unchanged. Update
`apps/web/dist/` only if implementation changes the static bundle.
...
[contracts/reconciliation-comparison-tables.md](./contracts/reconciliation-comparison-tables.md),
...
- Traceability: PASS. The UI contract and data model require compared sources,
  fields, raw values, normalized values where available, status, severity, and
  stable evidence basis for visible comparison rows.
