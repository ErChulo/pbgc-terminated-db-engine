# Research: BSRS Recalculation Pattern Hardening

## Decision: Use the approved recalculation BSRS sample as the canonical validation source
...
committed in the repository. The recalculation sample under
`artifacts/reference/approved-samples/bsrs-config/recalculations/` is therefore
the reviewed source for section sequence and line-cluster evidence.

**Alternatives considered**:

- Derive recalculation patterns from raw PBGC guidance or source documents:
  rejected because deterministic engine modules must not read raw or unreviewed
  source material.
- Invent new recalculation sections outside the approved sample: rejected
...
**Rationale**: Statement block-pattern hardening already established a small
internal helper, sample parsing, deterministic sorting, and structured semantic
finding payloads for `bsrs_configuration_output`. Recalculation hardening should
reuse that boundary rather than creating a new package, output adapter, or
persistence layer.

**Alternatives considered**:

- Add a new recalculation validation package: rejected because the scope is
...
## Decision: Validate recalculation sequence and row roles together

**Rationale**: The approved recalculation sample includes semantic marker rows,
support/detail rows, narrative text, subtotal-like lines, formatting-only rows,
and spacers. Validation must classify these roles so formatting rows do not
create false missing-section findings while orphan semantic rows still produce
structured warnings or errors.
...
- Validate only file shape and headers: rejected because existing
  ...
  semantic recalculation-pattern hardening.
- Treat every nonblank row as semantic evidence: rejected because approved
  samples include formatting and narrative rows that must be distinguished from
...
payloads. Recalculation findings must carry source path, row/block reference,
block family, section context, line-cluster evidence, rule version, producing
module, severity, and finding code so repeated runs can be compared
byte-for-byte.
