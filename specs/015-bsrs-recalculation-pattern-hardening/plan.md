# Implementation Plan: BSRS Recalculation Pattern Hardening

**Branch**: `[014-bsrs-recalculation-pattern-hardening]` | **Date**: 2026-05-21 | **Spec**: [spec.md](./spec.md)

**Input**: Feature specification from `/specs/015-bsrs-recalculation-pattern-hardening/spec.md`

## Summary

Deliver backend recalculation-pattern semantic hardening for
`bsrs_configuration_output` only. The increment validates approved
recalculation block patterns from the committed BSRS sample configuration
artifacts, including expected section sequencing, recalculation-specific row
role classification, structured findings, and repeated-run stability. It
preserves existing contracts, browser-only sql.js boundaries, and current
successful BSRS output behavior.

## Technical Context

**Language/Version**: TypeScript for browser-compatible deterministic helpers
and Vitest regression coverage; approved BSRS recalculation sample
...
validation helpers, committed approved recalculation sample in
`artifacts/reference/approved-samples/bsrs-config/recalculations/`, existing
...
writes. Recalculation-pattern validation evidence is test-local or uses
existing BSRS trace/output rows only where current contracts already require
them.

**Testing**: Regression tests for approved recalculation block patterns,
expected recalculation section sequence, recalculation-specific row role
classification, malformed recalculation findings, repeated-run finding shape,
...
**Performance Goals**: Recalculation-pattern validation over the committed
approved recalculation sample completes within normal local regression-suite
...
**Scale/Scope**: BSRS recalculation-pattern semantic validation for the
committed approved recalculation sample configuration files only. Recalculation
...
- Reviewed input boundary: PASS. Validation consumes approved repository
  ...
  output.
- Traceability: PASS. Recalculation-pattern findings require source path,
  row/block reference, block family, section context, line-cluster evidence,
  rule version, producing module, warning/error code, and semantic category.
...
```text
specs/015-bsrs-recalculation-pattern-hardening/
├── plan.md
├── research.md
├── data-model.md
├── quickstart.md
├── contracts/
│   └── bsrs-recalculation-pattern-validation.md
├── checklists/
│   └── requirements.md
└── spec.md
```
...
**Structure Decision**: Keep recalculation-pattern hardening in the existing
...
recalculation evidence. Do not create `packages/output-adapters/*`, new
...
[contracts/bsrs-recalculation-pattern-validation.md](./contracts/bsrs-recalculation-pattern-validation.md),
...
- Reviewed input boundary: PASS. Validation sources are approved recalculation
  BSRS samples, existing deterministic outputs, and existing contracts only.
- Traceability: PASS. The recalculation-pattern validation contract records
  source path, row/block reference, block family, section context, line-cluster
