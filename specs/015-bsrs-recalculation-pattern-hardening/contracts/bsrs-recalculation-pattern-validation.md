# Contract: BSRS Recalculation Pattern Validation

## Scope

This contract defines internal backend validation behavior for recalculation
...
- Recognize approved recalculation section markers and line clusters from
  committed recalculation sample artifacts.
- Validate required section presence, relative order, and supporting line
  clusters.
- Distinguish formatting-only and narrative rows from semantic recalculation
  evidence.

### Recalculation Row Role Classification

- Classify rows as semantic markers, support rows, detail rows, subtotals,
  narrative rows, formatting rows, or spacers.
- Ensure formatting-only and spacer rows do not create false missing-section
  findings.
- Emit findings for orphan semantic rows that lack approved recalculation
  section context.

### Section Sequence and Stability

- Validate source-path stable recalculation section ordering.
...
Each recalculation-pattern validation finding must include:
...
Recalculation-pattern validation must not change:
