# Quickstart: Cross-Slice Value Reconciliation Hardening

## Scope

This increment plans backend value-level validation and regression protection
across the already implemented BSRS configuration, V1/VE, and
valuation-listings output slices. It uses approved sample artifacts, DD.csv,
current committed output evidence, existing contracts, and the existing
cross-slice reconciliation foundation already in the repository.
...
```bash
npm test -- packages/tests/hardening-cross-slice-value-reconciliation.test.ts
```

Run the existing cross-slice reconciliation foundation checks with:
...
- Selected shared numeric, categorical, identifier, form, required, and nullable
  values reconcile across current BSRS, V1/VE, and valuation-listing evidence.
...
- Mismatches include severity and basis metadata.
- Approved nullable, optional, unsupported, and formatting-only cases do not
  create false blocking findings.
