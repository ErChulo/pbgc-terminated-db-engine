...
export function resolveV1VeOutput(packet: V1VeOutputPacket, inputPacketId: string, ruleVersion: string): ResolvedV1VeOutput {
  ...
  if (override) {
    if (!validateOverrideFieldName(override.output_column_name)) {
      return { row, warnings };
    }
    (row as Record<string, string | number | boolean | null>)[override.output_column_name] = override.override_value;
    warnings.push({
      code: "TECHNICAL_OVERRIDE_APPLIED",
      message: `Technical override applied to ${override.output_column_name}`,
      field_name: override.output_column_name,
